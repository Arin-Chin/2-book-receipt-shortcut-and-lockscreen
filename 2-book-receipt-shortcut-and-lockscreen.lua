...
-- 名称：阅读小票 + 锁屏壁纸（阅读日票 / 墨痕壁纸 / 菜单留单台，按勾选池随机或轮流）
-- 功能：
--   · 手势（QuickLook 分发动作）调出阅读小票，随机 / 轮流展示已勾选的样式
--   · 休眠锁屏（screensaver_type = book_receipt）与手势共用同一勾选池与摘要显示模式，均可作锁屏壁纸
--   · 墨痕壁纸引擎整合自 inkstain.koplugin，已彻底剔除 Miuread（觅阅）相关代码，
--     数据源固定为 KOReader 阅读统计，渲染为内存 BlitBuffer（渲染路径不写盘、无定时器；加载期会复制墨痕字体到 fonts/ 目录；无网络）
--     墨痕壁纸“设备”栏每次渲染实时读取当前设备型号/分辨率/DPI（-- 修改：v2.6.0）
...
--           与「阅读日票 → 锁屏背景」共用「背景图片显示方式」（适应屏幕/拉伸填满/居中不缩放）；
--           取图失败时显示友好提示而不是回退到封面屏保；该样式不依赖当前书籍（文件管理器休眠也可用）。
--   v2.7.14 新增“唤醒刷新计数”诊断日志（info，不改行为）：唤醒后 3 秒内统计各来源（widget/mode）
--           发给 UIManager.setDirty 的请求并汇总一行，用于两台设备对比定位唤醒闪烁差异。
--   v2.7.13 修复「锁屏背景=随机图片 + 背景图片显示方式=居中（不缩放）」看起来是黑底：
--           居中模式图片按原始尺寸画在正中，而票面也在正中——图片比屏幕小时整块被票面盖住，
--           屏幕上只剩填充色（若 KOReader「边框填充」=黑色就是全黑）；
--           现在该模式下先按原始尺寸探测一次：铺不满屏幕（或大于 2 倍屏幕）就自动改用「适应屏幕」，
--           并记一条 info 日志；另修一个潜在崩溃/使用已释放内存：书籍封面底图来自 bookinfo 缓存，
--           原先未标 image_disposable=false，widget 释放时会连带释放缓存里的封面。
--           另：墨痕壁纸设置里的「设备：…」栏上移到「摘要显示模式」下方（本级）。
--   v2.7.12 墨痕壁纸「设备：…」栏可自定义输入：点那一栏弹出输入框，预填当前（自动/自定义）文字，
--           留空保存即恢复自动读取（Device:info() + 分辨率 + DPI）；自定义文字对墨痕与菜单样式
--           都生效（后者是「点单设备：」行）；另给锁屏背景加了取图日志
--           （随机图片会打印实际选中的文件路径，取图失败会给 WARN），便于定位“背景不显示”。
--   v2.7.11 阅读日票设置精简（仅菜单）：删除两项原版胶片遗留、对日票无效的项——
--           「内容」（内容模式/高亮与进度/随机）与「设置休眠状态显示文字」；
--           其设置键（CONTENT_MODE_* / SLEEP_TEXT / 高亮常量）、汉化条目、以及只服务高亮模式的
--           5 个死辅助函数（含孤立的 datetime require）一并清掉；
--           「背景」改名「锁屏背景」（该项只作用于锁屏，手势弹窗不吃它）；
--           保留「封面缩放」——同属胶片遗留、日票也不读取，本次未要求删除。
--   v2.7.10 以「可选开关」形式恢复唤醒合并重绘（默认关，不干预 KOReader 默认行为）：
--           阅读摘要设置 → 唤醒合并重绘；打开后唤醒期把各来源的刷新请求全部吞掉，
--           静默 0.25s（硬上限 2.5s）后统一重绘一次（ui 波形），一次唤醒只更新一屏；
--           关闭（默认）时三个包装器（Screensaver.close / UIManager.setDirty）一行都不生效。
--   v2.7.9  撤销全部「针对闪烁」的修改——那三处闪烁（手势弹窗 / 熄屏 / 唤醒）的触发机制都在
--           KOReader 侧（UIManager 的 partial→full 提升、ScreenSaverWidget 的刷新、唤醒时多来源
--           重绘），不再与它对抗：
--           ① 手势弹窗恢复 KOReader 默认刷新（显示不带 region；关闭回到整屏 full）；
--           ② 锁屏恢复整屏 full 刷新（删除「锁屏刷新」设置、自动波形、ScreenSaverWidget:onShow 屏蔽）；
--           ③ 唤醒不再接管 UIManager.setDirty（删除 drop/downgrade/合并、唤醒诊断日志、
--              Screensaver.close 包装与 Screen.refreshFull 记录）——v2.7.10 以默认关闭的开关形式
--              恢复了其中“合并重绘”一项，其余仍不再接管。
--           保留：阅读日票渲染器、两个手势动作、锁屏不露当前页的底色兜底、诗词惰性解析、
--                 底图拉伸上限与居中/适应屏幕的底色修复。
--   v2.7.8  修复「背景图片显示方式=居中（不缩放）/适应屏幕」时锁屏露出当前阅读页：
--           底图没铺满屏幕（居中、留边、取图失败）时，ScreenSaverWidget 没有底色，
--           票面与底图之外就直接是屏幕原内容（＝当前阅读页）；v2.7.3 只处理了「完全无底图」的情况。
--           现改为：只要没有显式底色就补不透明填充（默认白，尊重 KOReader「边框填充=黑」），
--           与 KOReader 原版屏保先 Screen:clear() 白屏的意图一致，但不多一次全屏刷新。
--           同时修一个潜在崩溃：ImageWidget 的 stretch_limit 分支会算 width/height，
--           “居中”模式没有 width/height，故该参数只在拉伸模式传。
--   v2.7.7  底图沿用 KOReader 自己的「拉伸上限」设置（screensaver_stretch_limit_percentage），
--           避免小图被无限放大后发糊（未设置时无上限，行为不变）。
--   v2.7.4  诗词 200 条不再在补丁加载期解析（阻塞启动/重启）：改为首次渲染墨痕/菜单
--           样式时才解析（parsePoemText 自带缓存）；菜谱/名言原本已是首次使用才解析。
--   v2.7.3  锁屏不再露出当前阅读页 + 锁屏诊断日志：
--           背景设置选“透明”时，票面之外原本保留屏幕原内容（＝当前阅读页），
--           锁屏看起来就像“显示当前页”；现睡眠路径在底色/底图均缺失时统一兜底白底，
--           手势弹窗不受影响（仍可透明）；
--           另新增一条 info 级锁屏渲染日志（样式/背景设置/实际底色/构建耗时），
--           便于从 crash.log 定位休眠闪烁。
--   v2.7.2  手势动作拆分与改名：
--           原「阅读摘要」动作改名「Book-RL：阅读摘要」（event 仍为 QuickLook）；
--           新增专用动作「Book-RL：阅读日票」（event = ShowReadingTicket → ReaderUI:onShowReadingTicket），
--           无视勾选池与摘要显示模式，恒呼出「阅读日票」（force_style 绕过 getEffectiveStyle，
--           不推进轮换序号），与原先独立补丁 2-read-receipt.lua 的手势语义一致。
--           不推进轮换序号），与原先独立补丁 2-read-receipt.lua 的手势语义一致。
--   v2.7.0  胶片票根(film) 渲染替换为「阅读日票」(READ RECEIPT)：
--           移植 2-read-receipt.lua 的票面渲染器（圆角+锯齿+撕口+伪条码+星级只读展示），
--           样式键沿用 film，勾选池/旧配置迁移/锁屏降级逻辑零改动，仅用户可见名称改为「阅读日票」；
--           日票不响应评分点击（手势弹窗与锁屏点击仍为关闭）；
--           film 专属的「内容模式/背景/封面缩放/休眠文字」设置项保留，但新票不再读取
--           （内容模式、封面缩放、休眠文字无对应版位；背景仍由屏保路径作为票面外底色/底图生效）。
--   v2.6.0  菜单按摘要类型重构（胶片票根 / 墨痕壁纸 / 菜单留单台 / 摘要显示模式）：
--           三个样式菜单各带勾选框，勾选=纳入显示池；摘要显示模式简化为「轮流显示 / 随机显示」，
--           固定效果由“仅勾选一种样式”自然实现；旧版 fixed/random/alternate 与 book_receipt_random_style
--           自动派生勾选状态（随机/轮流→全选，固定→仅该样式），无需手动迁移；
--           手势与锁屏改回共用同一池与模式（移除 v2.5.0 的锁屏独立键）；
--           胶片票根：封面缩放/休眠文字从“内容”提升为一级项；
--           背景随机图片改用系统默认自定义图片文件夹（屏保设置 screensaver_dir）；
--           完全去除小花（✿）按钮与 Reading Insights 广播；
--           墨痕壁纸设备栏实时读取当前设备（Device:info() 优先，回退 Device.model），
--           并在墨痕壁纸设置内新增只读设备信息行（text_func 每次打开菜单刷新）。
...
do
    local zh_translations = {
        ...
        ["Book receipt"] = "阅读摘要",
        -- -- 修改：v2.7.2 手势动作名统一加 Book-RL 前缀，并新增专用「阅读日票」手势
        ["Book-RL: Book receipt"] = "Book-RL：阅读摘要",
        ["Book-RL: Reading ticket"] = "Book-RL：阅读日票",
        ["Show book receipt on sleep screen"] = "在休眠屏幕显示阅读摘要",
        ["Sleep screen background"] = "锁屏背景",
        ["White fill"] = "白色背景",
        ["Transparent"] = "透明",
        ["Black fill"] = "黑色背景",
        ["Random image"] = "随机图片",
        ...
        ["Style"] = "显示风格",
        -- 修改：v2.6.0 菜单按摘要类型重构——三个样式菜单各自勾选，摘要显示模式二选一
        ["Film strip"] = "阅读日票",
        ["Ink stain"] = "墨痕壁纸",
        ...
        ["Film strip (fixed style)"] = "阅读日票（固定风格）",
        -- 胶片版(film，现为阅读日票)特有硬编码翻译
        ["CURRENT PAGE"] = "当前页码",
        ["READ"] = "已读",
        ["TODAY"] = "今日",
        ["MIN"] = "分钟",
...
local K = {
    ...
    -- 显示样式（阅读日票 / 墨痕壁纸 / 菜单留单台）
    STYLE_SETTING = "book_receipt_style",             -- 旧版固定样式（仅作勾选状态迁移回退用）
    RANDOM_STYLE = "book_receipt_random_style",       -- 旧版随机开关（仅作勾选状态迁移回退用）
    STYLE_MODE_SETTING = "book_receipt_style_mode",   -- 摘要显示模式：random 随机 / alternate 轮流（旧值 fixed 视为 random）
    STYLE_TOGGLE_STATE = "book_receipt_style_toggle_state",  -- 轮流模式当前状态（手势/锁屏共享一条轮换序列）
    -- 修改：v2.6.0 三个样式各自是否纳入显示池（勾选状态）
    STYLE_ENABLED = {
        ["film"] = "book_receipt_film_enabled",
        ["inkstain"] = "book_receipt_inkstain_enabled",
        ["menu"] = "book_receipt_menu_enabled",
        ["image"] = "book_receipt_image_enabled",
    },
    ...
    STYLE_FILM = "film",       -- 胶片票根（v2.7.0 起渲染为阅读日票，样式键不变）
...
-- ========== 通用辅助函数 ==========
-- 背景随机图片目录：改为使用系统默认的自定义图片文件夹（屏保设置里的 screensaver_dir），
-- 与 KOReader 原生“随机图片”屏保同一目录（-- 修改：v2.6.0）
local function pickRandomReceiptBackgroundImage()
    local dir = G_reader_settings:readSetting("screensaver_dir")
    if not dir or dir == "" or lfs.attributes(dir, "mode") ~= "directory" then return nil end
    local files = {}
...
-- 居中模式是否不适用（-- 修改：v2.7.13，纯函数，便于单测）：
--   ① 图片比屏幕小：居中后整块正好落在票面之下，屏幕上只剩填充色（用户看到“背景没生效”）；
--   ② 图片远大于屏幕（>2 倍）：1:1 渲染并常驻要吃掉几十 MB 内存。
-- 两种情况都改用“适应屏幕”（保持比例缩放），保证底图一定看得见。
local function shouldAdaptCenteredBackground(img_w, img_h, screen_w, screen_h)
    if img_w < screen_w or img_h < screen_h then return true end
    if img_w > screen_w * 2 or img_h > screen_h * 2 then return true end
    return false
end
...
local function buildBackgroundImageWidget(image_source)
    ...
    if type(image_source) == "string" then
        ...
        -- -- 修改：v2.7.13 传进来的 BlitBuffer（书籍封面）来自 bookinfo 缓存，不能由本 widget 释放
        image_opts.image_disposable = false
    end
    if mode == "stretch" then
        image_opts.width = screen_w; image_opts.height = screen_h
        -- -- 修改：v2.7.7 与原版屏保对齐：尊重 KOReader 自己的「拉伸上限」设置，
        -- 避免小图被无限放大后发糊（未设置时无上限，行为不变）。
        -- 注意：ImageWidget 的 stretch_limit 分支会算 self.width / self.height，
        -- 所以在没有给定 width/height 的“居中”模式下绝不能传（会 nil 算术报错）。
        image_opts.stretch_limit_percentage = G_reader_settings:readSetting("screensaver_stretch_limit_percentage")
    ...
    if mode == "center" then
        -- 不缩放：不给 width/height，getSize() 返回图片原生尺寸，再由 CenterContainer 居中；
        -- 但铺不满屏幕时会被票面整个盖住（只剩填充色），过大时又会吃内存——都退回“适应屏幕”
        local probe = image_widget:getSize() -- 强制渲染，拿到原生尺寸
        if shouldAdaptCenteredBackground(probe.w, probe.h, screen_w, screen_h) then
            image_widget:free()
            logger.info(LOG_TAG, string.format(
                "锁屏背景：居中不缩放不适用（图片 %dx%d，屏幕 %dx%d），改用适应屏幕",
                probe.w, probe.h, screen_w, screen_h))
            local fit_opts = { alpha = true, file_do_cache = false,
                width = screen_w, height = screen_h, scale_factor = 0 }
            if image_opts.file then
                fit_opts.file = image_opts.file
            else
                fit_opts.image = image_opts.image; fit_opts.image_disposable = false
            end
            return ImageWidget:new(fit_opts)
        end
...
local function getReceiptBackground(ui)
    ...
    elseif choice == "random_image" then
        ...
        if image_path then
            -- -- 修改：v2.7.12 记下实际选中的底图，便于定位“锁屏背景不显示/被遮住”
            logger.info(LOG_TAG, "锁屏背景图片：", tostring(image_path))
            local widget = buildBackgroundImageWidget(image_path)
            if widget then return nil, widget end
        end
        logger.warn(LOG_TAG, "锁屏背景：随机图片取图失败（screensaver_dir 为空或目录无效）")
        return nil, nil
    elseif choice == "book_cover" then
        local cover_bb = getActiveDocumentCover(ui)
        if cover_bb then
            logger.info(LOG_TAG, "锁屏背景：当前书籍封面")
            local widget = buildBackgroundImageWidget(cover_bb)
            if widget then return nil, widget end
        end
        logger.warn(LOG_TAG, "锁屏背景：取书籍封面失败，回退填充色")
        return nil, nil
    end
...
-- 锁屏底色兜底（-- 修改：v2.7.3，v2.7.8 扩展到「底图未铺满」）：
-- 睡眠屏不接受“无底色”。不只是背景选“透明”时：底图没铺满屏幕时（居中不缩放、适应屏幕留边、
-- 取图失败）票面与底图之外会保留屏幕原内容（＝当前阅读页面）。KOReader 原版屏保对此的做法是
-- 先 Screen:clear() 白屏再画图；本补丁改为直接给 ScreenSaverWidget 一个不透明填充，
-- 既避开了那次多余的全屏刷新，也不会露出书页；颜色尊重 KOReader 自己的「边框填充」设置。
-- 手势弹窗不经过本函数，仍可透明。
local function resolveScreensaverBackground(bg_color)
    if bg_color then return bg_color end
    if G_reader_settings:readSetting("screensaver_img_background") == "black" then
...
-- ========== 样式工具 ==========
-- 历史遗留的样式名（editorial/dashboard/receipt）一律归入 film（现渲染为阅读日票），
-- 当前支持三种样式：阅读日票 / 墨痕壁纸 / 菜单样式（留台单），三者同池随机。
local function normalizeReceiptStyle(value)
    if value == "editorial" or value == "dashboard" or value == "receipt" then
        -- 旧版卡片样式已移除，统一收敛为 film（现渲染为阅读日票）
        return K.STYLE_FILM
    elseif value == K.STYLE_FILM or value == K.STYLE_INKSTAIN or value == K.STYLE_MENU
        or value == K.STYLE_IMAGE then
        return value
    end
    -- 未知样式默认回退为 film（现渲染为阅读日票），保证任何脏配置都不会崩溃
    return K.STYLE_FILM
end

-- 全部可选样式（顺序即轮流顺序：阅读日票 → 墨痕壁纸 → 菜单留单台）
...
-- 摘要显示模式：只保留「随机 / 轮流」两种（-- 修改：v2.6.0）
-- 旧值 fixed 归入 random：固定效果由「显示池内仅勾选一种样式」自然实现
local function getStyleMode()
    if G_reader_settings:readSetting(K.STYLE_MODE_SETTING) == "alternate" then
        return "alternate"
    end
    return "random"
end

-- 旧版配置派生勾选状态（未显式写过勾选键时用于平滑迁移）：
--   · 旧版 random/alternate：三样式全部在池内
--   · 旧版 fixed（或无配置）：仅 STYLE_SETTING 指向的那一种在池内
local function getLegacyStyleEnabled(style)
    local mode = G_reader_settings:readSetting(K.STYLE_MODE_SETTING)
    if mode == "random" or mode == "alternate" then
        return true
    end
    if mode == "fixed" then
        return normalizeReceiptStyle(G_reader_settings:readSetting(K.STYLE_SETTING)) == style
    end
    -- 未设过 style_mode：再回退更旧的“随机开关”
...
local function isStyleEnabled(style)
    local key = K.STYLE_ENABLED[style]
    if G_reader_settings:has(key) then
        return G_reader_settings:isTrue(key)
    end
    return getLegacyStyleEnabled(style)
end

local function setStyleEnabled(style, enabled)
    G_reader_settings:saveSetting(K.STYLE_ENABLED[style], enabled and true or false)
end

-- 当前显示池：仅含已勾选的样式；全部取消勾选时回退阅读日票，保证恒有可用样式
local function getEnabledStyles()
    local list = {}
    for _, s in ipairs(getAllStyles()) do
        if isStyleEnabled(s) then list[#list + 1] = s end
    end
    if #list == 0 then list = { K.STYLE_FILM } end
    return list
end

-- 轮流模式：在“已勾选样式”中顺序推进（自动跳过未勾选项），状态持久化于 G_reader_settings
local function getAlternateStyleAndAdvance(styles)
    ...
    if not idx then
        idx = 1 -- 首次使用 / 脏数据 / 原样式已被取消勾选：回落到池首项
    end
    G_reader_settings:saveSetting(K.STYLE_TOGGLE_STATE, styles[(idx % #styles) + 1])
    return styles[idx]
end

-- 依据「勾选池 + 摘要模式」返回本次应展示的样式（手势与锁屏共用同一池与模式）
local function getEffectiveStyle()
    local styles = getEnabledStyles()
    if #styles == 1 then return styles[1] end -- 仅勾选一种：等价固定
    if getStyleMode() == "alternate" then
        return getAlternateStyleAndAdvance(styles)
    end
    return styles[math.random(#styles)]
end


-- ========== 阅读日票（READ RECEIPT）渲染器 ==========
-- -- 修改：v2.7.0 移植自 2-read-receipt.lua，替换原「胶片票根」渲染：
--   · 样式键沿用 "film"（勾选池/旧配置迁移/锁屏判断零改动），仅用户可见名称改为「阅读日票」
--   · 只读展示：星级取自 doc_settings.summary.rating，点击不评分（手势弹窗与锁屏均由外层组件接管点击=关闭）
--   · 票面为内存 BlitBuffer 直绘，不写盘、无网络、无定时器
-- ---------------------------------------------------------------- geometry --
local BASE_W, BASE_H = 851.38, 1506.06
local current_scale = 1

local R = 35
local NOTCH_Y = 967.85
local NOTCH_DX = 3
local SCALLOPS = 8
...
local COL = {
    ...
    COVER = Blitbuffer.Color8(0x91),
    TRACK = Blitbuffer.Color8(0xC7),
    FILL = Blitbuffer.Color8(0x4C),
}
...
local STAR_GAP = 2
local STAR_X = 36.92
local STAR_TOP = 631.18
local RATING_TEXT_GAP = 8
local RATING_TEXT_SIZE = 30
local RATING_LABELS = { "很差", "较差", "一般", "还好", "完美" }
local RATING_DEFAULT_TEXT = "点评"
local STAR_TOUCH_EXPAND = 20
local MIN_STAR_SIZE = 8

-- 文字排版数据
local ITEMS = {
  {x=759.55,top=132.58,f="C",size=22,ls=1.16,c="K",t="状态"},
  {x=599.56,top=173.7,f="C",size=104,ls=7.67,c="K",t="在读",key="status"},
  {x=0,top=66.64,f="L",size=25,ls=-0.1,c="K",t="NO. D20260906",key="no",no_align=true},
  {x=35.04,top=89.3,f="C",size=25,ls=-1.25,c="K",t="阅读日票"},
  {x=34.12,top=136,f="C",size=72,ls=0.03,c="K",t="READ RECEIPT"},
  {x=34.36,top=232.58,f="C",size=25,ls=-0.78,c="K",t="今日阅读轨迹"},
  {x=193,top=232.92,f="L",size=25,ls=0,c="K",t="/"},
  {x=214.52,top=232.2,f="L",size=25,ls=-0.73,c="K",t="TODAY"},
  {x=287.64,top=236.2,f="L",size=25,ls=0,c="K",t="’"},
  {x=304.32,top=232.68,f="L",size=25,ls=0,c="K",t="S"},
  {x=321.6,top=232.2,f="L",size=25,ls=-0.26,c="K",t="READING"},
  {x=434.64,top=232.2,f="L",size=25,ls=-0.42,c="K",t="TRAIL"},
  {x=70.08, top=346.35, f="C", size=20, ls=0.07, c="K", t="开始日期"},
  {x=0,    top=340,   f="L", size=32,   ls=0,    c="K", t="·", dot_center=true},
  {x=169.37,top=346.24, f="L", size=20, ls=-0.2, c="K", t="DATE"},
  {x=319.24,top=344.75,f="C",size=20,ls=-0.12,c="K",t="今日星期"},
  {x=0,top=340,f="L",size=32,ls=0,c="K",t="·",dot_center=true},
  {x=418.24,top=346.24,f="L",size=20,ls=-0.51,c="K",t="DAY"},
  {x=583,top=344.51,f="C",size=20,ls=0.47,c="K",t="当天日期"},
  {x=0,top=340,f="L",size=32,ls=0,c="K",t="·",dot_center=true},
  {x=683.41,top=346.24,f="L",size=20,ls=0.18,c="K",t="DATE"},
  {x=70.08,top=371.53,f="L",size=36,ls=-0.61,c="K",key="start_date"},
  {x=317.76,top=371.13,f="C",size=34,ls=0.92,c="K",key="weekday_cn"},
  {x=397.4,top=371.37,f="C",size=34,ls=0,c="K",t="・"},
  {x=441.16,top=371.69,f="L",size=36,ls=-0.97,c="K",key="weekday_en"},
  {x=582.24,top=371.53,f="L",size=36,ls=-0.61,c="K",key="date"},
  {x=33.6,top=489.58,f="C",size=30,ls=0.49,c="K",t="当前阅读 READING"},
  {x=33.6,top=536.78,f="L",size=38,ls=-0.07,c="K",key="title",maxw=460},
  {x=33.6,top=583.98,f="L",size=30,ls=0.67,c="K",key="author",prefix="作者：",maxw=400},
  {x=33.6,top=716.58,f="C",size=30,ls=0.71,c="K",key="chapter",maxw=460},
  {x=33.6,top=763.78,f="C",size=30,ls=0.82,c="K",key="est_time",prefix="预计阅读时间：",maxw=500},
  {x=33.6,top=858.18,f="L",size=28,ls=-0.11,c="K",key="pages"},
  {x=148.72,top=857.52,f="C",size=28,ls=0,c="K",t="页",follow=10.8},
  {x=457.2,top=858.18,f="L",size=28,ls=-0.21,c="K",t="15%",key="pct", right_align=true},
  {x=39.12,top=1001.4,f="C",size=36,ls=-0.39,c="K",t="今日摘要"},
  {x=200.64,top=1001.78,f="L",size=36,ls=-0.2,c="K",t="SUMMARY"},
  {x=192,top=1077.44,f="C",size=28,ls=2.8,c="K",t="今日"},
  {x=604.12,top=1076.96,f="C",size=28,ls=0.52,c="K",t="总计"},
  {x=35.08,top=1115.83,f="L",size=26,ls=-0.08,c="K",t="PAGES READ"},
  {x=35.08,top=1147.87,f="C",size=26,ls=2.2,c="K",t="阅读页数"},
  {x=35.08,top=1202.29,f="L",size=40,ls=-0.35,c="K",key="d_pages"},
  {x=86.2,top=1200.61,f="C",size=40,ls=0,c="K",t="页",follow=2.3},
  {x=221.08,top=1115.83,f="L",size=26,ls=-0.02,c="K",t="READING TIME"},
  {x=221.08,top=1147.87,f="C",size=26,ls=2.29,c="K",t="阅读时长"},
  {x=221.08,top=1201.65,f="L",size=40,ls=0.32,c="K",key="d_time"},
  {x=444.52,top=1116.31,f="L",size=26,ls=-0.11,c="K",t="PAGES READ"},
  {x=444.52,top=1148.35,f="C",size=26,ls=2.12,c="K",t="阅读页数"},
  {x=444.52,top=1203.01,f="L",size=40,ls=-0.35,c="K",key="total_pages"},
  {x=495.4,top=1201.33,f="C",size=40,ls=0,c="K",t="页",follow=2.1},
  {x=634.36,top=1116.31,f="L",size=26,ls=-0.04,c="K",t="READING TIME"},
  {x=634.36,top=1148.35,f="C",size=26,ls=2.29,c="K",t="阅读时长"},
  {x=634.36,top=1202.37,f="L",size=40,ls=0.32,c="K",key="total_time"},
  {x=0,top=1396.2,f="C",size=20,ls=0.03,c="K",key="id",prefix="阅读记录 · READING LOG · ",maxw=600,center=true},
}
...
local function disc(bb, ox, oy, cx, cy, r, c)
    local s = current_scale
    local scaled_r = r * s
    for dy = -ceil(scaled_r), ceil(scaled_r) do
        local hw = scaled_r * scaled_r - dy * dy
        if hw > 0 then
            hw = sqrt(hw)
            rect(bb, ox, oy, cx - hw/s, cy + dy/s, hw*2/s, 1/s, c)
        end
    end
end

local function dottedH(bb, ox, oy, x1, x2, y, dot_radius, gap, offset, c)
    local s = current_scale
    local period = (dot_radius * 2 + gap) * s
    local t = x1 * s - (offset or 0) * s
    local x1_s = x1 * s
    local x2_s = x2 * s
    local y_s = y * s
    local dot_r_s = dot_radius * s
    while t < x2_s do
        local cx = t + dot_r_s
        if cx >= x1_s and cx <= x2_s then
            disc(bb, ox, oy, cx/s, y, dot_radius, c)
        end
        t = t + period
    end
end
...
local function scallopCentres()
    local t = {}
    for k = 0, SCALLOPS do t[#t + 1] = k * (BASE_W / SCALLOPS) end
    return t
end
...
local function colSpans(x)
    ...
    if x > BASE_W - R + NOTCH_DX then
        local d = BASE_W + NOTCH_DX - x
...
-- 缓存每行/列的扫描区间，避免重复计算
local paper_spans_cache = {}
local function getPaperSpans(scale)
    local key = string.format("%.2f", scale)
    if paper_spans_cache[key] then
        return paper_spans_cache[key]
    end

    local s = scale
    local W_s = floor(BASE_W * s + 0.5)
    local H_s = floor(BASE_H * s + 0.5)

    local rows = {}
    for y_s = 0, H_s - 1 do
        local y_base = y_s / s
        rows[y_s] = rowSpans(y_base)
    end

    local cols = {}
    for x_s = 0, W_s - 1 do
        local x_base = x_s / s
        cols[x_s] = colSpans(x_base)
    end

    local cache = { rows = rows, cols = cols, W_s = W_s, H_s = H_s }
    paper_spans_cache[key] = cache
    return cache
end

local function paintPaper(bb, ox, oy)
    local s = current_scale
    local cache = getPaperSpans(s)
    local rows, cols = cache.rows, cache.cols
    local W_s, H_s = cache.W_s, cache.H_s

    -- 绘制水平方向（白色内部 + 左右边界黑点）
    for y_s = 0, H_s - 1 do
        local spans = rows[y_s]
        for _, span in ipairs(spans) do
            local x1_base, x2_base = span[1], span[2]
            local x1_s = floor(x1_base * s + 0.5)
            local x2_s = floor(x2_base * s + 0.5)
            if x2_s > x1_s then
                bb:paintRect(ox + x1_s, oy + y_s, x2_s - x1_s, 1, Blitbuffer.COLOR_WHITE)
                bb:paintRect(ox + x1_s, oy + y_s, 1, 1, Blitbuffer.COLOR_BLACK)
                bb:paintRect(ox + x2_s - 1, oy + y_s, 1, 1, Blitbuffer.COLOR_BLACK)
            end
        end
    end
    ...
    -- 绘制垂直方向边界黑点（不重复绘制白色）
    for x_s = 0, W_s - 1 do
        local spans = cols[x_s]
        for _, span in ipairs(spans) do
            local y1_base, y2_base = span[1], span[2]
            local y1_s = floor(y1_base * s + 0.5)
            local y2_s = floor(y2_base * s + 0.5)
            if y2_s > y1_s then
                bb:paintRect(ox + x_s, oy + y1_s, 1, 1, Blitbuffer.COLOR_BLACK)
                bb:paintRect(ox + x_s, oy + y2_s - 1, 1, 1, Blitbuffer.COLOR_BLACK)
            end
        end
    end
end
...
local function rawFace(which, size)
    ...
    if not ok or not face then
        logger.warn(LOG_TAG, "failed to get built-in font", font_name, "- falling back to cfont")
...
local function calibrate()
    ...
    FONT_SCALE = guess / 72
    logger.info(LOG_TAG, "font scale calibrated to", FONT_SCALE)
end

local function getFace(which, size)
    if not FONT_SCALE then calibrate() end
    return rawFace(which, size * FONT_SCALE * current_scale)
end
...
local function faceHasChar(face, ch)
    ...
    elseif b1 < 0xF0 then
        code = (b1 - 0xE0) * 0x10000 + ((ch:byte(2) or 0) - 0x80) * 0x40 + ((ch:byte(3) or 0) - 0x80)
...
local function drawReceiptText(bb, x, baseline, face, alt_face, text, ls, c)
    ...
    for _, ch in ipairs(utf8chars(text)) do
        ...
        x = x + textWidth(f, ch) + (ls or 0) * current_scale
    end
    return x - (ls or 0) * current_scale
end
...
local function collectData(ui)
    ...
    local d = {
        ...
        weekday_cn = wd_cn[wd],
        weekday_en = wd_en[wd],
        no = "NO. D" .. os.date("%Y%m%d", now),
        id = "D" .. os.date("%Y%m%d", now),
        rating = 0,
        chapter = nil,
        total_pages = "0",
        total_time = "0 m",
    ...
    if ui and ui.document then
        ...
        if toc and toc.getTocTitleByPage then
            local chapter = toc:getTocTitleByPage(cur) or ""
            if chapter ~= "" then
                d.chapter = chapter
            end
        end
    ...
    if ui and ui.doc_settings then
        ...
        if ok and summary then
            ...
            if summary.rating then
                d.rating = math.floor(tonumber(summary.rating) or 0)
            end
        end
    end
    if d.progress and d.progress >= 0.999 then d.status = "已读" end
    ...
    elseif not enabled then
        ...
        if stats.getPageTimeTotalStats and id_book then
            ...
            if ok then
                if pages then d.total_pages = tostring(pages) end
                if secs then d.total_time = hm(secs) end
            end
        end

        if d.total_time == "0 m" and stats.book_read_time then
            d.total_time = hm(stats.book_read_time)
        end

        if stats.start_current_period then
            local since = tonumber((tostring(stats.start_current_period):gsub("[^%d]", ""))) or os.time()
            d.s_time = hm(os.time() - since)
        end
        ...
        if id_book then
            local ok_db, err = pcall(function()
                ...
                if first then d.start_date = os.date("%Y.%m.%d", first) end
    ...
    local avg_time_per_page = stats and stats.avg_time
    if avg_time_per_page and avg_time_per_page > 0 and ui and ui.document then
        local total_pages = ui.document:getPageCount() or 0
        local current_page = ui.view and ui.view.state and ui.view.state.page or 1
        local page_left = math.max(total_pages - current_page, 0)
        d.est_time = hm(avg_time_per_page * page_left)
    else
        d.est_time = "--"
    end
...
local function loadCover(doc)
    ...
    for i, fn in ipairs(tries) do
        ...
        if ok and res then
            logger.info(LOG_TAG, "cover loaded via strategy", i)
            return res
        end
        logger.warn(LOG_TAG, "cover strategy", i, "->", tostring(res))
    end
    return nil
end
...
local function blitCoverInto(bb, img, dx, dy, dw, dh)
    ...
    if dw < 1 or dh < 1 then return false end
    ...
    logger.warn(LOG_TAG, "ImageWidget cover failed:", tostring(err))
    local scaled = RenderImage:scaleBlitBuffer(img, dw, dh, false)
    if scaled then
        bb:blitFrom(scaled, dx, dy, 0, 0, dw, dh)
        return true
    end
    return false
end
...
local Receipt = InputContainer:extend{
    ...
    scale = 1,
    star_icons_full = nil,
    star_icons_empty = nil,
}

function Receipt:init()
    self.dimen = Geom:new{ x = 0, y = 0, w = Screen:getWidth(), h = Screen:getHeight() }
    if not self.data then
        self.data = collectData(self.ui)
    end
    self.rating = self.data and self.data.rating or 0
    local screen_w, screen_h = Screen:getWidth(), Screen:getHeight()
    self.scale = math.min(screen_w * 0.68 / BASE_W, screen_h * 0.90 / BASE_H)

    self.star_icons_full = {}
    self.star_icons_empty = {}
    local star_size_px = math.max(MIN_STAR_SIZE, math.floor(STAR_SIZE * self.scale + 0.5))
    local star_gap_px = math.floor(STAR_GAP * self.scale + 0.5)

    for i = 1, STAR_COUNT do
        local ok_full, icon_full = pcall(IconWidget.new, IconWidget, {
            icon = "star.full",
            width = star_size_px,
            height = star_size_px,
            alpha = true,
        })
        if ok_full and icon_full then
            self.star_icons_full[i] = icon_full
        else
            logger.warn(LOG_TAG, "failed to create full star icon #", i)
        end

        local ok_empty, icon_empty = pcall(IconWidget.new, IconWidget, {
            icon = "star.empty",
            width = star_size_px,
            height = star_size_px,
            alpha = true,
        })
        if ok_empty and icon_empty then
            self.star_icons_empty[i] = icon_empty
        else
            logger.warn(LOG_TAG, "failed to create empty star icon #", i)
        end
    end
end

function Receipt:onCloseWidget()
    if self.star_icons_full then
        for _, w in ipairs(self.star_icons_full) do
            if w then pcall(function() w:free() end) end
        end
    end
    if self.star_icons_empty then
        for _, w in ipairs(self.star_icons_empty) do
            if w then pcall(function() w:free() end) end
        end
    end
end
...
function Receipt:paintTo(bb, x, y)
    ...
    local ox = x + px((Screen:getWidth() - BASE_W * current_scale) / 2)
    local oy = y + px((Screen:getHeight() - BASE_H * current_scale) / 2)

    paintPaper(bb, ox, oy)

    dottedH(bb, ox, oy, 33.7, 815, 327.2, 2, 7, 2, COL.K)
    dottedH(bb, ox, oy, 33.7, 815, 418.6, 2, 7, 2, COL.K)
    rect(bb, ox, oy, 586.06, 53.52, 1, 259.9, COL.K)
    rect(bb, ox, oy, 273.3, 339.9, 1, 66, COL.K)
    rect(bb, ox, oy, 540.4, 339.9, 1, 66, COL.K)

    local function paintCover()
        local cx, cy, cw, ch = px(512.9), px(489.58), px(302.9), px(399.6)
        local abs_cx = ox + math.floor(cx * current_scale)
        local abs_cy = oy + math.floor(cy * current_scale)
        local abs_cw = math.floor(cw * current_scale)
        local abs_ch = math.floor(ch * current_scale)
        if not blitCoverInto(bb, self:getCover(), abs_cx, abs_cy, abs_cw, abs_ch) then
            bb:paintRect(abs_cx, abs_cy, abs_cw, abs_ch, COL.COVER)
        end
        rect(bb, ox, oy, cx, cy, cw, 1, COL.K)
        rect(bb, ox, oy, cx, cy + ch - 1, cw, 1, COL.K)
        rect(bb, ox, oy, cx, cy, 1, ch, COL.K)
        rect(bb, ox, oy, cx + cw - 1, cy, 1, ch, COL.K)
    end
    pcall(paintCover)

    rect(bb, ox, oy, 33.6, 816.68, 423.6, 14.6, COL.TRACK)
    local progress = (self.data and self.data.progress) or 0.1705
    rect(bb, ox, oy, 33.6, 816.68, 423.6 * progress, 14.6, COL.FILL)
    dottedH(bb, ox, oy, 33.2, 814.5, 972.85, 2, 9, 2, COL.K)
    rect(bb, ox, oy, 31.9, 1058.35, 785.8, 1, COL.K)
    rect(bb, ox, oy, 45.3, 1092.1, 137, 1, COL.K)
    rect(bb, ox, oy, 253.8, 1092.1, 137.5, 1, COL.K)
    rect(bb, ox, oy, 456.8, 1092.1, 137, 1, COL.K)
    rect(bb, ox, oy, 664.8, 1092.1, 137.5, 1, COL.K)
    -- 竖线三条：恢复原始硬编码参数
    rect(bb, ox, oy, 200.9, 1122.2, 1, 126.3, COL.K)
    rect(bb, ox, oy, 422.4, 1122.2, 1, 126.3, COL.K)
    rect(bb, ox, oy, 612.4, 1122.2, 1, 126.3, COL.K)
    dottedH(bb, ox, oy, 33.2, 814.5, 1277.35, 2, 9, 2, COL.K)
    for _, b in ipairs(BARS) do
        rect(bb, ox, oy, b[1], BARCODE_Y + 5, b[2], BARCODE_H, COL.K)
    end
    disc(bb, ox, oy, 383.28, 1446.84, 3.1, COL.K)
    for i, cx in ipairs({ 406.92, 427.8, 448.44, 469.32 }) do
        disc(bb, ox, oy, cx, 1446.84, 2.4, COL.K)
    end
    ...
    for _, it in ipairs(ITEMS) do
        local text = it.t
        if it.key and data[it.key] then
            if it.prefix then
                text = it.prefix .. (data[it.key] or "")
            else
                text = data[it.key]
            end
        end
    ...
    -- 计算状态值右边缘（用于“状态”右对齐）
    local status_right_edge_base = nil
    for _, it in ipairs(ITEMS) do
        if it.key == "status" and widths[it] then
            status_right_edge_base = it.x + widths[it] / current_scale
            break
        end
    end

    -- 计算“今日摘要”的基线（用于SUMMARY对齐）
    local summary_baseline = nil
    for _, it in ipairs(ITEMS) do
        if it.t == "今日摘要" then
            summary_baseline = oy + (it.top + K_CN * it.size) * current_scale
            break
        end
    end

    local title_left_idx = 6
    local title_slash_idx = 7
    local title_right_indices = {8, 9, 10, 11, 12}
    local x_start = 34.36

    local left_item = ITEMS[title_left_idx]
    local left_face = getFace(left_item.f, left_item.size)
    local left_text = left_item.t
    local left_baseline = oy + (left_item.top + K_CN * left_item.size) * current_scale
    local end_x = drawReceiptText(bb, ox + x_start * current_scale, left_baseline, left_face, nil, left_text, left_item.ls * current_scale, COL.K)
    local x_pos = (end_x - ox) / current_scale

    local slash_item = ITEMS[title_slash_idx]
    local slash_face = getFace(slash_item.f, slash_item.size)
    local slash_text = slash_item.t
    local slash_baseline = oy + (slash_item.top + K_LA * slash_item.size) * current_scale
    local slash_x = x_pos
    end_x = drawReceiptText(bb, ox + slash_x * current_scale, slash_baseline, slash_face, nil, slash_text, slash_item.ls * current_scale, COL.K)
    x_pos = (end_x - ox) / current_scale

    for _, idx in ipairs(title_right_indices) do
        local item = ITEMS[idx]
        local face = getFace(item.f, item.size)
        local text = item.t
        local baseline = oy + (item.top + K_LA * item.size) * current_scale
        end_x = drawReceiptText(bb, ox + x_pos * current_scale, baseline, face, nil, text, item.ls * current_scale, COL.K)
        x_pos = (end_x - ox) / current_scale
    end
    ...
    for idx, it in ipairs(ITEMS) do
        ...
        if text ~= "" then
            ...
            if it.maxw and textWidth(face, text) > it.maxw * current_scale then
                local chars = utf8chars(text)
                while #chars > 1 and textWidth(face, table.concat(chars) .. "…") > it.maxw * current_scale do
                    table.remove(chars)
                end
            ...
            -- 特殊处理“状态”标签右对齐
            if it.t == "状态" and status_right_edge_base then
                sx = ox + (status_right_edge_base - widths[it] / current_scale) * current_scale
            elseif it.t == "SUMMARY" then
                -- 紧跟“今日摘要”后，留一个空格宽度
                local space_w = textWidth(face, " ")
                sx = pen + space_w
            elseif it.no_align then
                local state_item = ITEMS[1]
                local status_item = ITEMS[2]
                local state_width = widths[state_item] or 0
                local status_width = widths[status_item] or 0
                local right_state = 759.55 + state_width / current_scale
                local right_status = 599.56 + status_width / current_scale
                local max_right = math.max(right_state, right_status)
                sx = ox + max_right * current_scale - tw
            elseif it.center then
                sx = ox + (BASE_W * current_scale - tw) / 2
            elseif it.right_align then
                sx = ox + it.x * current_scale - tw
            elseif it.dot_center then
                local left_item = ITEMS[idx-1]
                local right_item = ITEMS[idx+1]
                if left_item and right_item then
                    local left_x = left_item.x * current_scale
                    local left_width = widths[left_item] or 0
                    local right_x = right_item.x * current_scale
                    local mid = (left_x + left_width + right_x) / 2
                    sx = ox + mid - tw/2
                else
                    sx = ox + it.x * current_scale
                end
            else
                sx = it.follow and (pen + it.follow * current_scale) or (ox + it.x * current_scale)
            end

            local baseline
            if it.t == "SUMMARY" and summary_baseline then
                baseline = summary_baseline  -- 强制使用中文基线
            else
                local k = it.f == "C" and K_CN or K_LA
                baseline = oy + (it.top + k * it.size) * current_scale
            end

            local ok, res = pcall(drawReceiptText, bb, sx, baseline,
                                  face, alt, text, it.ls * current_scale, COL[it.c])
            if ok then
                pen = res
            else
                logger.warn(LOG_TAG, "draw failed", text, res)
            end
        end
        ::continue::
    end

    local rating = self.rating or 0
    local star_x = ox + STAR_X * current_scale
    local star_y = oy + STAR_TOP * current_scale
    local star_size_scaled = math.max(MIN_STAR_SIZE, STAR_SIZE * current_scale)
    local star_gap_scaled = STAR_GAP * current_scale

    for i = 1, STAR_COUNT do
        local icon
        if i <= rating then
            icon = self.star_icons_full and self.star_icons_full[i]
        else
            icon = self.star_icons_empty and self.star_icons_empty[i]
        end

        if icon then
            local ok_paint = pcall(function()
                icon:paintTo(bb, math.floor(star_x + 0.5), math.floor(star_y + 0.5))
            end)
            if not ok_paint then
                logger.warn(LOG_TAG, "star paint failed for", i <= rating and "full" or "empty", "icon", i)
            end
        else
            logger.warn(LOG_TAG, "missing star icon for index", i)
        end

        star_x = star_x + star_size_scaled + star_gap_scaled
    end

    local rating_text = RATING_DEFAULT_TEXT
    if rating > 0 and rating <= #RATING_LABELS then
        rating_text = RATING_LABELS[rating]
    end
    local full_text = "（" .. rating_text .. "）"
    local text_face = getFace("C", RATING_TEXT_SIZE)
    local text_baseline = star_y + (star_size_scaled - K_CN * RATING_TEXT_SIZE * current_scale) / 2 + K_CN * RATING_TEXT_SIZE * current_scale
    pcall(drawReceiptText, bb, star_x + RATING_TEXT_GAP * current_scale, text_baseline, text_face, nil, full_text, 0, COL.K)
end

-- 胶片票根(film) 构建入口：返回阅读日票 widget；无活动文档时返回 nil（由调用方降级）
local function buildFilmReceipt(ui, state)
    if not hasActiveDocument(ui) then return nil end
    local ok, widget = pcall(function()
        return Receipt:new{ ui = ui, document = ui.document, data = collectData(ui) }
    end)
    if not ok or not widget then
        logger.warn(LOG_TAG, "阅读日票构建失败：", tostring(widget))
        return nil
    end
    return widget
end
...
-- 诗词库惰性化（-- 修改：v2.7.4）：诗词 200 条原先在补丁加载期就解析（阻塞 KOReader
-- 启动/重启），而它只在渲染墨痕/菜单样式时才用到；现改为首次使用时解析
-- （parsePoemText 自带 poem_cache，后续调用零成本）。菜谱/名言原本已如此。
logger.info(LOG_TAG, "补丁已加载（诗词解析推迟到首次渲染）")
...
-- 入参：无（状态存于 G_reader_settings，序号从 1..#parsePoemText() 循环推进）
...
local function pickQuote()
    local quotes = parsePoemText() -- -- 修改：v2.7.4 惰性化：首次渲染时才解析（内部已缓存）
...
-- -- 修改：v2.6.0 每次调用均实时读取当前设备（型号/分辨率/DPI），不缓存、不落盘，
--          换设备或旋转屏幕后重新渲染时自动更新（墨痕壁纸与设置菜单共用本函数）
local function getDeviceInfoString(prefix)
    prefix = prefix or "设备："
    -- 0) 自定义文字优先（-- 修改：v2.7.12）：墨痕壁纸菜单里点“设备：…”那一栏即可输入；
    --    留空 = 恢复自动读取。前缀（“设备：”/“点单设备：”）由调用方传入，自定义时也保留。
    local custom = G_reader_settings:readSetting(K.DEVICE_TEXT_SETTING)
    if type(custom) == "string" and custom ~= "" then
        return prefix .. custom
    end
    -- 1) 设备型号：优先官方 Device:info()（即 self.model），异常/为空时回退 Device.model
    local model
    local ok_info, info = pcall(function() return Device:info() end)
    if ok_info and type(info) == "string" and info ~= "" then
        model = info
    end
    if not model then
    ...
    if not model then model = "未知设备" end
...
-- 构建墨痕壁纸 / 菜单样式 Widget（全屏图；-- 修改：v2.6.0 去除底部小花按钮）
-- 入参：style 当前样式（K.STYLE_INKSTAIN / K.STYLE_MENU；-- 修改：v2.4.5 新增菜单样式）
local function buildInkStainWidget(ui, style)
...
--   · film：阅读日票（内部含 hasActiveDocument 检查）
...
-- 随机图片样式（-- 修改：v2.7.15）：整屏显示 KOReader 默认图片文件夹（screensaver_dir）里的一张随机图片。
-- 与「阅读日票 → 锁屏背景 = 随机图片」的区别：那个是垫在票面**之下**，这个是**整屏壁纸**
-- （即 KOReader 原生“随机图片屏保”的效果），并使用同一套「背景图片显示方式」。
local function buildImageStyleWidget()
    local image_path = pickRandomReceiptBackgroundImage()
    if not image_path then
        logger.warn(LOG_TAG, "随机图片样式：取图失败（screensaver_dir 为空或目录无效）")
        return buildInkStainFallbackWidget(_("无法显示随机图片，请检查屏保的随机图片文件夹"))
    end
    logger.info(LOG_TAG, "随机图片样式：", tostring(image_path))
    local widget = buildBackgroundImageWidget(image_path)
    if not widget then
        return buildInkStainFallbackWidget(_("无法显示随机图片，请检查屏保的随机图片文件夹"))
    end
...
local function buildReceipt(ui, state, style)
    if style == nil then
        style = getEffectiveStyle()
        logger.dbg(LOG_TAG, "buildReceipt: style 未传参，回退样式解析")
    end
    if style == K.STYLE_IMAGE then
        return buildImageStyleWidget()
    end
    if style == K.STYLE_INKSTAIN or style == K.STYLE_MENU then
        return buildInkStainWidget(ui, style)
    end
    return buildFilmReceipt(ui, state)
end
...
function quicklookbox:init()
    ...
    -- -- 修改：v2.7.2 force_style（专用手势「阅读日票」）绕过勾选池与摘要显示模式，恒渲染指定样式
    local style = self.force_style or getEffectiveStyle()
    self.style = style
    local receipt_widget = buildReceipt(self.ui, self.state, style)
...
-- 注册分发器动作（-- 修改：v2.7.2 标题加 Book-RL 前缀，并新增专用「阅读日票」手势）
Dispatcher:registerAction("quicklookbox_action", {
    category="none",
    event="QuickLook",
    title=_("Book-RL: Book receipt"),
    reader=true,
})

-- -- 修改：v2.7.2 新增专用动作：无视勾选池/摘要显示模式，恒呼出「阅读日票」
Dispatcher:registerAction("reading_ticket_action", {
    category="none",
    event="ShowReadingTicket",
    title=_("Book-RL: Reading ticket"),
    reader=true,
})

-- 弹窗呼出公共入口（-- 修改：v2.7.2 从 ReaderUI:onQuickLook 抽出，force_style 供专用手势使用）
local function showQuickLook(ui, force_style)
...
function ReaderUI:onShowReadingTicket()
    showQuickLook(self, K.STYLE_FILM)
end
...
local function describeRefreshWidget(widget)
    ...
    if widget == "all" then return "all" end
    if type(widget) == "table" then return tostring(widget.name or widget.id or widget) end
...
wakeDiagFinish = function()
    ...
    wake_diag_open = false
    logger.info(LOG_TAG, string.format("唤醒刷新计数：n=%d（%s）", wake_diag_total,
        table.concat(wake_diag_list, ", ")))
end
local function wakeDiagFinishHard() wakeDiagFinish() end

Screensaver.close = function(self, ...)
    -- 诊断窗口（不影响行为）：记录本次唤醒收到的刷新请求
    wake_diag_open = true
    wake_diag_total = 0
    wake_diag_list = {}
...
if orig_ui_set_dirty then
    UIManager.setDirty = function(self, widget, refreshtype, refreshregion, refreshdither)
        if wake_diag_open then
            wake_diag_total = wake_diag_total + 1
            if #wake_diag_list < 12 then
...
Screensaver.show = function(self)
    ...
    -- 墨痕/菜单样式基于 KOReader 全局统计，无需打开书籍即可锁屏；阅读日票必须存在活动文档
    -- （-- 修改：v2.4.5 判断扩展为墨痕 + 菜单两种基于统计的样式）
    -- 修改：v2.6.0 锁屏与手势共用同一条“勾选池 + 摘要显示模式”
    local style = getEffectiveStyle()
    -- 随机图片样式（-- 修改：v2.7.15）同样不依赖当前书籍，可在文件管理器休眠
    local is_stat_based = style == K.STYLE_INKSTAIN or style == K.STYLE_MENU or style == K.STYLE_IMAGE
    -- 阅读日票(film)必须存在活动文档；若无则降级为墨痕(inkstain)渲染，
    ...
    -- 修改：v2.6.0 手势与锁屏共用同一条“勾选池 + 摘要显示模式”，锁屏不再单独解析样式
    local receipt_widget = buildReceipt(ui, state, style)

    if receipt_widget then
        local background_color, background_widget = getReceiptBackground(ui)
        -- -- 修改：v2.7.15 随机图片样式本身就是整屏图片：不要再叠一层底图，只保留填充色兜底
        if style == K.STYLE_IMAGE then
            background_widget = nil
        end
        -- 旧"黑底终端"样式的黑色背景特判已随该样式移除；
        -- 墨痕本身为白底全屏排版，背景设置仅对阅读日票生效，此处无需再按样式特判
        -- -- 修改：v2.7.3 锁屏不留透明底：背景=“透明”（或随机图片/封面取图失败）时，
        -- 票面之外一直是屏幕原内容（＝当前阅读页），等于锁屏显示当前页；
        -- 睡眠路径统一兜底白底，锁屏永不露出书页（手势弹窗不受影响，仍可透明）。
        local bg_choice = G_reader_settings:readSetting(K.BG_SETTING) or "white"
        background_color = resolveScreensaverBackground(style == K.STYLE_IMAGE and nil or background_color)
        -- -- 修改：v2.7.3 保留一条锁屏渲染日志（样式/背景/实际底色/构建耗时），便于从 crash.log 排查
        logger.info(LOG_TAG, string.format(
            "锁屏渲染：样式=%s 背景设置=%s 实际底色=%s 构建耗时=%.0fms 刷新=full",
            tostring(style), tostring(bg_choice),
            background_widget and "底图" or (background_color == Blitbuffer.COLOR_BLACK and "黑" or "白"),
            (os.clock() - t_build) * 1000))
...
_G.dofile = function(filepath)
    ...
    if filepath and filepath:match("screensaver_menu%.lua$") then
        ...
        if result and result[1] and result[1].sub_item_table then
            ...
            -- 修改：v2.6.0 菜单按摘要类型重构：
            -- · 一级项：阅读日票 / 墨痕壁纸 / 菜单留单台 / 摘要显示模式
            -- · 前三个样式各带勾选框（点左侧方框切换是否纳入显示池，点文字进入各自设置）
            -- · 摘要显示模式只保留「轮流显示 / 随机显示」，固定效果由仅勾选一种样式实现
            local function styleEnabledItem(text, style, sub_items)
                return {
                    text = text,
                    checked_func = function() return isStyleEnabled(style) end,
            ...
            -- 纯勾选项（无子设置，如菜单留单台）：点击整行切换
            local function styleToggleItem(text, style)
                return {
                    text = text,
                    checked_func = function() return isStyleEnabled(style) end,
                    callback = function()
            ...
            local function displayModeItem(text, mode, help_text)
                return {
                    text = text,
            ...
            -- 封面缩放（-- 修改：v2.6.0 从“内容”子菜单提升为胶片票根一级项；v2.7.0 起阅读日票不读取）
            local cover_scale_item = {
                text = _("Cover scale"),
                keep_menu_open = true,
            ...
            -- 背景（阅读日票使用；墨痕壁纸自带白底全屏排版）
            -- 随机图片目录已改为系统默认的自定义图片文件夹（屏保设置 screensaver_dir，-- 修改：v2.6.0）
            -- 锁屏背景（-- 修改：v2.7.11 原“背景”；只影响锁屏，手势弹窗不吃这个设置）
            local background_menu = {
                text = _("Sleep screen background"),
                sub_item_table = {
            ...
            -- 墨痕壁纸：统计周期 / 书单数量（与菜单留单台共用同一组配置键）
            local stats_period_item = {
            ...
            local device_info_item = {
                ...
                callback = function(touchmenu_instance)
                    local current = G_reader_settings:readSetting(K.DEVICE_TEXT_SETTING)
                    if type(current) ~= "string" or current == "" then
                        -- 预填当前自动值（无前缀），方便在它基础上改
                        local ok, auto = pcall(getDeviceInfoString, "")
                        current = (ok and auto) or ""
                    end
                    local input_dialog
                    input_dialog = InputDialog:new{
                        title = _("Device row text"),
                        input = current,
                        buttons = {
                            {
                                {
                                ...
                                },
                                {
                                    ...
                                    is_enter_default = true,
                                    callback = function()
                                        local new_text = util.trim(input_dialog:getInputText() or "")
                                        if new_text == "" then
                                            G_reader_settings:delSetting(K.DEVICE_TEXT_SETTING)
                                        else
                                            G_reader_settings:saveSetting(K.DEVICE_TEXT_SETTING, new_text)
                                        end
                                        UIManager:close(input_dialog)
                                        if touchmenu_instance then touchmenu_instance:updateItems() end
                                    end,
            ...
            -- 三个样式一级菜单：勾选=纳入显示池；点左侧方框切换，点文字进入各自设置
            local film_menu = styleEnabledItem(_("Film strip"), K.STYLE_FILM, {
                background_menu,
                cover_scale_item,
            })
            local inkstain_menu = styleEnabledItem(_("Ink stain"), K.STYLE_INKSTAIN, {
                stats_period_item,
                top_n_item,
            })
            -- 菜单留单台与墨痕共用统计周期/书单数量，自身无独立设置，故为纯勾选项
            local menu_style_menu = styleToggleItem(_("Order Slip"), K.STYLE_MENU)

            -- 随机图片样式（-- 修改：v2.7.15）：整屏壁纸 = KOReader 默认图片文件夹里的一张随机图片；
            -- 与阅读日票的「锁屏背景」共用「背景图片显示方式」设置
            local image_style_menu = styleEnabledItem(_("Random image"), K.STYLE_IMAGE, {
                {
                    text = _("Background image placement"),
                    help_text = _("Shared with the reading ticket's lock-screen background."),
                    sub_item_table = {
                        genMenuItem(_("Fit to screen"), K.BG_IMAGE_MODE_SETTING, "fit"),
                        genMenuItem(_("Stretch to screen"), K.BG_IMAGE_MODE_SETTING, "stretch"),
                        genMenuItem(_("Center without scaling"), K.BG_IMAGE_MODE_SETTING, "center"),
            ...
            local display_mode_menu = {
                text = _("Summary display mode"),
                sub_item_table = {
                    displayModeItem(_("Alternate display"), "alternate",
                        _("Alternate display: checked styles are shown in turn (film → ink stain → menu).")),
                    displayModeItem(_("Random display"), "random",
                        _("Random display: a checked style is picked at random each time.")),
                },
            }
...
-- 修改：v2.5.0 新增，v2.6.0 随菜单重构更新暴露函数，v2.7.0 追加日票渲染入口（buildFilmReceipt/Receipt）；
-- 当且仅当设置 book_receipt_dev_test 为真时（默认不设置，生产永不触发），
-- 将纯逻辑函数引用暴露到 _G._book_receipt_style_test，供 spec/test_style_pool.js 断言。
if G_reader_settings and G_reader_settings:isTrue("book_receipt_dev_test") then
    _G._book_receipt_style_test = {
        ...
        getEnabledStyles = getEnabledStyles,
        isStyleEnabled = isStyleEnabled,
        setStyleEnabled = setStyleEnabled,
        ...
        buildFilmReceipt = buildFilmReceipt,
        Receipt = Receipt,
        resolveScreensaverBackground = resolveScreensaverBackground,
    ...
    logger.info(LOG_TAG, "已启用样式测试钩子（book_receipt_dev_test）")
end
