...
四种样式各带勾选框：**勾选＝纳入显示池**（**点左侧方框**或**长按整行**都可切换，点文字进入该样式设置）；`摘要显示模式` 选「轮流显示」按
...
| **摘要显示模式** | — | `轮流显示 / 随机显示` |
| **设备信息：…** | — | 墨痕/菜单的「设备」栏显示内容；**点这一栏可自定义输入**，留空保存即恢复自动读取（型号 + 分辨率 + DPI） |

> 样式项的勾选框只能用「点左侧方框」或「长按整行」切换：KOReader 的菜单把勾选框列固定在左侧，
> `TouchMenuItem` 没有右侧槽位，故补丁用官方支持的 `hold_callback`（长按）作为整行切换方式。
...
**Q：休眠 / 唤醒时屏幕会闪（黑闪）？**
A：这是 **KOReader 自身**的刷新行为，不是本补丁产生的（挂起→唤醒之间补丁没有任何渲染）。唤醒时 KOReader 会先后发多次整屏 `full` 刷新（`ScreenSaverWidget:onCloseWidget`、`Kindle:outofScreenSaver` 的重绘），e-ink 上 `full` 波形＝一次黑闪；插件在 resume 时的重排版还会再补几次。本补丁**不再干预**这一过程，只在 `crash.log` 里记一行诊断，方便对比两台设备：

```
[BookReceipt] 唤醒刷新计数：n=6（nil/full, ReaderUI/nil, bookshelf/full, all/full, ReaderUI/partial, ReaderUI/partial）
```

另可检查 `屏保 → 延迟屏幕更新`：设为「Never/关闭」时 KOReader 会多一次整屏重绘（其他取值会跳过它）。
